...
def decade_list(request):
    ...
    return render(request, 'tunr/decade_list.html', {'decades': decades})


def fad_list(request):
    fads = Fad.objects.all()
    return render(request, 'tunr/fad_list.html', {'fads': fads})


def decade_detail(request, pk):
    decade = Decade.objects.get(id=pk)
    return render(request, 'tunr/decade_detail.html', {'decade': decade})


def fad_detail(request, pk):
    fad = Fad.objects.get(id=pk)
    return render(request, 'tunr/fad_detail.html', {'fad': fad})
...
def decade_create(request):
    if request.method == 'POST':
        ...
        form = DecadeForm()
    return render(request, 'tunr/decade_form.html', {'form': form})
